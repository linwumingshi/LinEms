 ...
 *
 * <p>设备在线时，其连接锁 {@code mqtt:conn:{deviceKey}} 持有者是设备所在 Broker 节点
 * （TTL 60s，心跳续期，与 Broker 侧「连接锁 + 路由定位」双职责一致）。下行指令先查
 * owner：命中 → 定向投递 mqtt.down.{nodeId}；未命中（离线/竞态窗口）→ 回落 mqtt.broadcast
 * 由 Broker 幽灵订阅/离线队列兜底。</p>
 *
 * <p>异常降级：Redis 不可用时返回 null（走广播），不抛异常阻断下发。</p>
 */
@Slf4j
@Component
