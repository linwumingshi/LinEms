...
/**
 * 物模型只读投影（读取产品当前生效版本 + schema_json）。 缓存路径：cache:model:current:{pk}（L1 本地 + Redis）→ 本查询兜底。
 * <p>
 * 跨 schema 读 es_product.iot_product/iot_thing_model，单 DataSource 全限定名 （与 energy-alarm
 * ProductInfoMapper / energy-command DeviceInfoMapper 同模式）。
 * </p>
 */
@Mapper
