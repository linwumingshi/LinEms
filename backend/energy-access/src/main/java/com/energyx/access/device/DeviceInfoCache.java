 ...
 *
 * <p>
 * 上行热路径每报文一次查询，必须避开 MySQL；设备维度的 tenant/station/enterprise 稳定不变（归属变更属低频运维操作，走主动失效），30min
 * TTL 足够。
 * </p>
 */
@Slf4j
...
public class DeviceInfoCache {
	...
	public DeviceInfoCache(StringRedisTemplate redis, DeviceMapper deviceMapper, AccessProperties props,
	...
	public DeviceInfo get(String productKey, String deviceName) {
		...
		if (info != null) {
			try {
				redis.opsForValue()
					.set(key, objectMapper.writeValueAsString(info),
