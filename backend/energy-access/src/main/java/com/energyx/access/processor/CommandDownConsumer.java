 ...
 *
 * <p>
 * Phase 6 Command Center 产出指令 → 本服务桥接 → Broker 全节点 fan-out → 设备所在节点 按 QoS1 投递
 * down/command。离线补发：设备上线时 OfflineCommandRedeliverer 把 Redis 离线队列（iot:cmd:q）中的
 * CommandDownMessage 重新桥接，路径复用本方法。
 * </p>
 */
@Slf4j
@Component
public class CommandDownConsumer implements KafkaRecordHandler {
	...
	public void handle(ConsumerRecord<String, String> record) {
		...
		try {
			...
			log.warn("[Access] command-down 反序列化失败 key={} offset={}，进 DLQ", record.key(), record.offset());
