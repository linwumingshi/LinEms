package com.sanduo.energy.access.processor;

import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.core.type.TypeReference;
import com.sanduo.energy.access.config.AccessProperties;
import com.sanduo.energy.access.device.DeviceInfo;
import com.sanduo.energy.access.device.DeviceInfoCache;
import com.sanduo.energy.access.model.ModelValidator;
import com.sanduo.energy.access.model.ThingModel;
import com.sanduo.energy.access.model.ThingModelCache;
import com.sanduo.energy.access.publish.EventPublisher;
import com.sanduo.energy.common.kafka.KafkaRecordHandler;
import com.sanduo.energy.common.message.CommandAckMessage;
import com.sanduo.energy.common.message.LifecycleMessage;
import com.sanduo.energy.common.message.RawMessage;
import com.sanduo.energy.common.message.ThingEventMessage;
import com.sanduo.energy.common.message.ThingPropertyMessage;
import com.sanduo.energy.common.mqtt.MqttTopicInfo;
import com.sanduo.energy.common.mqtt.MqttTopicUtil;
import com.sanduo.energy.common.mqtt.MqttUpType;
 ...
 * </pre></p>
 *
 * <p>线程安全：无实例状态（所有依赖为线程安全 Bean），可供 KafkaConsumerEngine 多线程并发调用。</p>
 */
@Slf4j
@Component
public class UplinkProcessor implements KafkaRecordHandler {
    ...
    public UplinkProcessor(EventPublisher publisher, ThingModelCache modelCache,
                           DeviceInfoCache deviceInfoCache, MessageDedup messageDedup,
    ...
    public void handle(ConsumerRecord<String, String> record) throws Exception {
        ...
        if (!messageDedup.tryOnce("access", device.getDeviceId(), messageId,
                props.getMsgDedupTtlSeconds())) {
            return;
        }
    ...
    private void processProperty(DeviceInfo device, MqttTopicInfo info, byte[] payload, String messageId) {
        try {
            ...
            if (model == null) {
                log.warn("[Access] 物模型缺失 productKey={} deviceId={}，跳过属性标准化",
                        info.productKey(), device.getDeviceId());
                return;
            }
            ModelValidator.ValidationResult result = ModelValidator.validateProperties(model, reported);
            if (!result.valid()) {
                log.warn("[Access] 属性校验拒绝 deviceId={} messageId={} errors={}",
                        device.getDeviceId(), messageId, result.errors());
                return;
            }
    ...
    private void processEvent(DeviceInfo device, MqttTopicInfo info, byte[] payload, String messageId) {
        try {
            ...
            ThingModel model = modelCache.get(info.productKey());
            if (model == null) {
                log.warn("[Access] 物模型缺失 productKey={} deviceId={}，跳过事件标准化",
                        info.productKey(), device.getDeviceId());
                return;
            }
            ModelValidator.EventCheck check = ModelValidator.checkEvent(model, eventName);
            if (check == null) {
                log.warn("[Access] 未知事件 eventName={} deviceId={}（未在物模型登记）",
                        eventName, device.getDeviceId());
                return;
            }
    ...
    private void processAck(DeviceInfo device, byte[] payload, String messageId) {
        try {
            ...
            if (commandId.isEmpty()) {
                log.warn("[Access] ACK 缺 commandId deviceId={} messageId={}",
                        device.getDeviceId(), messageId);
                return;
            }
    ...
    private void traceRaw(String messageId, String deviceKey, Long deviceId,
                          RouterEnvelope envelope, String rejectReason) {
