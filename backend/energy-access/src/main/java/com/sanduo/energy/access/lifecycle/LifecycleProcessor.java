...
import com.sanduo.energy.access.mapper.DeviceStatusMapper;
import com.sanduo.energy.access.mapper.OnlineRecordMapper;
 ...
 * </ul>
 *
 * <p>在线权威状态仍在 Redis iot:online（Broker 心跳续期，30s 过期判离线），
 * 本服务落 MySQL 的是审计/近线视图，读写频率低（每设备每秒 1 次量级）。</p>
 */
@Slf4j
...
public class LifecycleProcessor {
    ...
    public LifecycleProcessor(DeviceStatusMapper deviceStatusMapper,
                              OnlineRecordMapper onlineRecordMapper,
                              OfflineCommandRedeliverer offlineRedeliverer,
    ...
    private void insertRecord(LifecycleMessage msg, int eventType, Date now) {
        onlineRecordMapper.insert(
                idGenerator.nextId(),
                msg.getDeviceId(),
                msg.getTenantId() == null ? 0L : msg.getTenantId(),
                eventType,
                msg.getReason(),
                msg.getIp(),
                msg.getBrokerNode(),
