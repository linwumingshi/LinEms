...
/**
 * /ws/alarm 握手鉴权（P0-2）：浏览器原生 WebSocket 无法携带 Authorization 头， token 经 URL
 * 查询参数传递（{@code ws://host/ws/alarm?token=&lt;jwt&gt;}）。
 *
 * <p>
 * 网关 GlobalAuthFilter 对 /ws 前缀只放行升级请求，真正的 JWT 验签下沉到这里： 验签通过 → 把 userId/username/tenantId
 * 写入握手 attributes（供 {@link AlarmWebSocketHandler#afterConnectionEstablished} 记录登录身份）；
 * 缺省/伪造/过期 → 401 拒绝握手。直连本服务绕过网关的路径同样被拒（防御纵深）。
 * </p>
 */
@Slf4j
@Component
