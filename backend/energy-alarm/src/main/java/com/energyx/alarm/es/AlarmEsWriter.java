 ...
 *
 * <p>
 * 写入为<b>尽力而为</b>：单线程异步队列 + 失败记日志，不阻塞告警主链路（告警权威源是 MySQL iot_alarm_record，ES
 * 仅作检索/大屏日志冗余）。索引/字段对齐 sql/elasticsearch/alarm_log.mapping.json。
 * </p>
 */
@Slf4j
@Component
	...
	private void write(String alarmEventId, Map<String, Object> doc) {
		...
		try {
			...
			if (response.statusCode() >= 300) {
				log.warn("[Alarm] ES 写入失败 index={} id={} status={} resp={}", index, alarmEventId, response.statusCode(),
						response.body());
			}
		}
		catch (Exception e) {
