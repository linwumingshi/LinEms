 ...
 *
 * <p>
 * 驾驶舱/运营大屏订阅后接收告警事件 JSON（与 Kafka iot-alarm 载荷一致）。 会话集合 CopyOnWriteArraySet
 * 线程安全；广播失败仅记录，不影响告警主链路。
 * </p>
 */
@Slf4j
@Component
public class AlarmWebSocketHandler extends TextWebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession session) {
		...
		log.info("[Alarm] WS 连接建立 session={} userId={} tenantId={} 当前在线={}", session.getId(), userId, tenantId,
				sessions.size());
	}
