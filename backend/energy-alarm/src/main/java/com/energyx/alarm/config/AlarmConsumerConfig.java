...
import com.energyx.alarm.consumer.EventAlarmConsumer;
import com.energyx.alarm.consumer.PropertyAlarmConsumer;
import com.energyx.alarm.service.AlarmKafkaPublisher;
...
public class AlarmConsumerConfig {
	...
	public KafkaConsumerEngine propertyEngine(PropertyAlarmConsumer handler, AlarmProperties props,
		...
		return start(new KafkaConsumerEngine(KafkaTopicConstant.IOT_THING_PROPERTY, "energy-alarm-prop", handler,
				baseProps(props), props.getConsumerThreads(), props.getPollMs(), publisher::send, props.getDlqTopic()));
	}

	@Bean(destroyMethod = "close")
	public KafkaConsumerEngine eventEngine(EventAlarmConsumer handler, AlarmProperties props,
			AlarmKafkaPublisher publisher) {
		return start(new KafkaConsumerEngine(KafkaTopicConstant.IOT_THING_EVENT, "energy-alarm-event", handler,
				baseProps(props), props.getConsumerThreads(), props.getPollMs(), publisher::send, props.getDlqTopic()));
	}
