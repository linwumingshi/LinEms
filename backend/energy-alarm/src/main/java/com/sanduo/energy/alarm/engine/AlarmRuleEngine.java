package com.sanduo.energy.alarm.engine;
 ...
 * </ul>
 *
 * <p>比较语义：GT/GTE/LT/LTE 仅接受数值（字符串可解析为数值时也参与比较，解析失败视为不满足）；
 * EQ/NEQ 两端均可解析为数值时按数值比较，否则按字符串比较。</p>
 */
public final class AlarmRuleEngine {
    ...
    public static boolean propertyMet(AlarmCondition c, Object current) {
        return c != null && c.getMetric() != null
                && c.getOp() != null && c.getValue() != null
