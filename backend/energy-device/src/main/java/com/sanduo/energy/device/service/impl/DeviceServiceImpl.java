package com.sanduo.energy.device.service.impl;
...
import com.sanduo.energy.device.entity.DeviceCredential;
import com.sanduo.energy.device.mapper.DeviceCredentialMapper;
import com.sanduo.energy.device.mapper.DeviceMapper;
import com.sanduo.energy.device.service.DeviceService;
import com.sanduo.energy.device.util.DeviceSecretGenerator;
 ...
 * 设备资产服务实现。
 *
 * <p>租户隔离：查询/更新/删除全部由条件化租户拦截器按 {@link TenantContext} 自动追加
 * {@code tenant_id} 条件（HTTP 线程），本服务无需显式按租户过滤；仅创建时从上下文读取
 * 租户写入。物化路径 {@code /父ID/子ID/} 与层级随 parentId 计算，供子树前缀查询。</p>
 */
@Slf4j
@Service
public class DeviceServiceImpl extends ServiceImpl<DeviceMapper, Device> implements DeviceService {
    ...
    public Long create(DeviceCreateReq req) {
        ...
        long dup = count(new LambdaQueryWrapper<Device>()
                .eq(Device::getProductKey, req.getProductKey())
        ...
        log.info("创建设备 deviceId={} name={} productKey={} type={} path={}", device.getDeviceId(),
                device.getDeviceName(), device.getProductKey(), device.getDeviceType(), device.getPath());
    ...
    public void update(Long deviceId, DeviceUpdateReq req) {
        ...
        if (req.getDeviceName() != null && !req.getDeviceName().equals(exists.getDeviceName())) {
            validateDeviceName(req.getDeviceName());
            long dup = count(new LambdaQueryWrapper<Device>()
                    .eq(Device::getProductKey, exists.getProductKey())
                    .eq(Device::getDeviceName, req.getDeviceName())
    ...
    public void delete(Long deviceId) {
        ...
        // 收集子树（path 前缀命中，含自身）
        List<Long> ids = list(new LambdaQueryWrapper<Device>()
                        .likeRight(Device::getPath, "/" + deviceId + "/")
                        .select(Device::getDeviceId))
                .stream().map(Device::getDeviceId).collect(Collectors.toList());
        ...
        credentialMapper.update(null, new LambdaUpdateWrapper<DeviceCredential>()
                .in(DeviceCredential::getDeviceId, ids)
    ...
    public IPage<Device> page(DeviceQuery query) {
        LambdaQueryWrapper<Device> wrapper = new LambdaQueryWrapper<Device>()
                ...
                .eq(query.getDeviceType() != null && !query.getDeviceType().isBlank(),
                        Device::getDeviceType, query.getDeviceType())
                ...
                .like(query.getKeyword() != null && !query.getKeyword().isBlank(),
                        Device::getDeviceName, query.getKeyword())
    ...
    public List<Device> tree(Long rootId) {
        ...
        if (rootId != null && rootId > 0) {
            ...
            all = list(new LambdaQueryWrapper<Device>()
                    .likeRight(Device::getPath, "/" + rootId + "/")
                    .orderByAsc(Device::getSort)
                    .orderByAsc(Device::getDeviceId));
        } else {
            all = list(new LambdaQueryWrapper<Device>()
    ...
    public CredentialView getCredential(Long deviceId) {
        ...
        DeviceCredential cred = credentialMapper.selectOne(new LambdaQueryWrapper<DeviceCredential>()
        ...
        return new CredentialView(deviceId, device.getDeviceName(),
                ...
                cred == null ? 0 : cred.getAuthStatus());
    }
    ...
    private List<Device> buildTree(List<Device> all) {
        Map<Long, Device> byId = all.stream()
