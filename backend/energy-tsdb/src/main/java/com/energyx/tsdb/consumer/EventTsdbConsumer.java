...
/**
 * 事件摄取：消费 iot-thing-event（key=deviceId）→ 边界幂等（stage=tsdb） → st_event 落库（payload JSON
 * 列）。幂等键用 messageId（事件去重/追踪实例级 由 eventId 承担，跨边界消费幂等仍以 messageId 为准）。
 */
@Slf4j
@Component
public class EventTsdbConsumer implements KafkaRecordHandler {
	...
	public EventTsdbConsumer(ObjectMapper objectMapper, MessageDedup dedup, TsdbBatchBuffer buffer,
