package com.energyx.tsdb.config;

import com.energyx.common.constant.KafkaTopicConstant;
import com.energyx.common.kafka.KafkaConsumerEngine;
import com.energyx.tsdb.consumer.EventTsdbConsumer;
import com.energyx.tsdb.consumer.PropertyTsdbConsumer;
import com.energyx.tsdb.kafka.TsdbKafkaProducer;
...
public class TsdbConsumerConfig {
	...
	public KafkaConsumerEngine propertyEngine(PropertyTsdbConsumer handler, TsdbProperties props,
		...
		return start(new KafkaConsumerEngine(KafkaTopicConstant.IOT_THING_PROPERTY, "energy-tsdb-prop", handler,
				baseProps(props), props.getConsumerThreads(), props.getPollMs(), producer::send, props.getDlqTopic()));
	}

	@Bean(destroyMethod = "close")
	public KafkaConsumerEngine eventEngine(EventTsdbConsumer handler, TsdbProperties props,
			TsdbKafkaProducer producer) {
		return start(new KafkaConsumerEngine(KafkaTopicConstant.IOT_THING_EVENT, "energy-tsdb-event", handler,
				baseProps(props), props.getConsumerThreads(), props.getPollMs(), producer::send, props.getDlqTopic()));
	}
