 ...
 *
 * <p>单连接 + 整语句批量执行：TDengine 原生支持一条语句内多个 INSERT 块
 * （自动建子表 + 多行多列），写入吞吐由批量缓冲的行数/字节阈值控制。
 * 连接失效时重建后重试一次，仍失败则上抛交由消费端进 DLQ。</p>
 *
 * <p>连接为进程级单例：TAOS-RS 连接可复用，写放大远小于 MySQL；后续若需
 * 更高吞吐可换 taos-spring-boot-starter 连接池，当前以简单可靠优先。</p>
 */
@Slf4j
@Component
    ...
    public void execute(String sql) throws Exception {
        ...
        while (true) {
            try {
                ...
                closeQuietly();
                if (attempt >= 2) {
                    log.error("[Tsdb] TDengine 写入失败（已重试） errorCode={} sqlState={} sql={}",
                            e.getErrorCode(), e.getSQLState(), abbreviate(sql), e);
                    throw e;
                }
