...
/**
 * 属性摄取：消费 iot-thing-property（key=deviceId，分区内保序）→ 边界幂等
 * （stage=tsdb）→ 构造宽表 SQL → 批量缓冲。
 *
 * <p>写入失败由缓冲上抛 → 引擎进 DLQ 并照常提交（at-least-once + 幂等兜底）。
 * 缺 messageId 的消息无法幂等，宁可丢弃不可重复落库（上游 access 保证必填）。</p>
 */
@Slf4j
@Component
public class PropertyTsdbConsumer implements KafkaRecordHandler {
    ...
    public PropertyTsdbConsumer(ObjectMapper objectMapper, MessageDedup dedup,
