 ...
 *
 * <p>
 * 纯静态、无 Spring 依赖：网关（WebFlux）与业务服务（Servlet）共用，保证 同一 secret/issuer 下签名与验签严格一致。算法由密钥长度决定
 * （{@code Keys.hmacShaKeyFor} 自动选型）：≥256bit → HS256，≥384bit → HS384， ≥512bit → HS512；dev
 * secret 48 字节=384bit，实际为 HS384。
 * </p>
 */
public final class JwtTokenUtil {
	...
	private static SecretKey secretKey(String secret) {
		if (secret == null || secret.isEmpty()) {
			throw new IllegalArgumentException("energyx.jwt.secret 未配置");
		}
		byte[] bytes = secret.getBytes(StandardCharsets.UTF_8);
		if (bytes.length < MIN_SECRET_BYTES) {
			throw new IllegalArgumentException(
					"energyx.jwt.secret 长度不足，HS 系列需 ≥ " + MIN_SECRET_BYTES + " 字节，当前 " + bytes.length);
		}
