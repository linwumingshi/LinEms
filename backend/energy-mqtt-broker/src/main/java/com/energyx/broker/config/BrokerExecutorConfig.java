 ...
 *
 * <p>
 * 原则（Phase 1 §4.5）：Netty IO 线程绝不做阻塞操作（认证 Redis/MySQL、会话持久化、 Kafka 生产都属于慢路径），必须剥离到独立业务线程池。
 * </p>
 *
 * <p>
 * 线程模型：
 ...
 *
 * <p>
 * 拒绝策略为「记日志丢弃」而非 CallerRunsPolicy：后者在队满时会让提交任务的 IO 线程内联执行阻塞任务，直接卡死整个 EventLoop。超载时丢一次持久化
 * （QoS 续传/离线队列退化为不可恢复）换取连接面不被打挂，是可接受的降级。
 * </p>
 */
@Slf4j
@Configuration
public class BrokerExecutorConfig {
	...
	public ExecutorService brokerExecutor() {
		...
		return new ThreadPoolExecutor(threads, threads, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<>(10_000),
				factory, handler);
	}

	/**
	 * 延迟任务调度线程（P1-11 Will Delay 延迟遗嘱投递等）。 独立单线程，避免延迟任务长时间 sleep 占用 brokerExecutor 工作线程。
	 */
