 ...
 *
 * <p>设备服务在吊销/禁用/重置凭据时 PUBLISH clientId 到该通道，Broker 收到后：
 * <ol>
 *   <li>删除 Redis 凭据缓存 cache:cred:{clientId}（吊销从最长 30min 缩到秒级生效）；</li>
 *   <li>关闭该设备的在线连接，强制重新认证（新凭据生效、旧凭据被拒）。</li>
 * </ol>
 * 基于 {@link RedisMessageListenerContainer}（专用 pub/sub 连接 + 断线重连）。</p>
 *
 * <p>发布方（设备服务侧，示意）：
 * {@code redisTemplate.convertAndSend("mqtt:cred:revoked", clientId)}。</p>
 */
@Slf4j
@Component
public class CredentialRevokeSubscriber {
    ...
    public CredentialRevokeSubscriber(RedisMessageListenerContainer container,
