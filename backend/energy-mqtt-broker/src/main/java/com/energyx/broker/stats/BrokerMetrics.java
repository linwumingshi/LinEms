 ...
 *
 * <p>
 * 计数器以 {@link BrokerStats} 的 AtomicLong 为数据源（FunctionCounter 弱引用读取）， 与
 * {@code /internal/broker/stats} 端点共享同一份计数，不产生双写不一致。
 * </p>
 *
 * <p>
 * 指标清单：
 ...
 * <li>broker_executor_queue_size / threads_active / threads_pool /
 * tasks_completed_total</li>
 * <li>mqtt_uplink_puback_latency_seconds（直方图：PUBLISH→Kafka 确认→PUBACK 端到端时延）</li>
 * </ul>
 * JVM 指标由 actuator 自动注册（jvm_memory/jvm_gc/process 等）。
 * </p>
 */
@Component
	...
	public BrokerMetrics(MeterRegistry registry, BrokerStats stats, SessionRegistry sessionRegistry,
		...
		if (brokerExecutor instanceof ThreadPoolExecutor tpe) {
			Gauge.builder("broker.executor.queue.size", tpe, e -> e.getQueue().size())
				.description("业务线程池队列深度")
				.register(registry);
			Gauge.builder("broker.executor.threads.active", tpe, ThreadPoolExecutor::getActiveCount)
				.description("业务线程池活跃线程数")
				.register(registry);
			Gauge.builder("broker.executor.threads.pool", tpe, ThreadPoolExecutor::getPoolSize)
				.description("业务线程池线程数")
				.register(registry);
			FunctionCounter.builder("broker.executor.tasks.completed", tpe, ThreadPoolExecutor::getCompletedTaskCount)
				.description("业务线程池累计完成任务数")
				.register(registry);
		}
