...
public class KafkaTopicInitializer {
    ...
    private void ensureTopics() throws Exception {
        ...
        try (AdminClient admin = AdminClient.create(props)) {
            ...
            addIfNeed(admin, existing, toCreate, toExpand,
                    KafkaTopicConstant.MQTT_DOWN_PREFIX + properties.getNodeId(),
    ...
    private void addIfNeed(AdminClient admin, Set<String> existing, List<NewTopic> toCreate,
                           java.util.Map<String, org.apache.kafka.clients.admin.NewPartitions> toExpand,
                           String topic, int partitions) throws Exception {
        if (!existing.contains(topic)) {
            toCreate.add(new NewTopic(topic, partitions, (short) 1));
            return;
        }
        int current = admin.describeTopics(java.util.List.of(topic)).allTopicNames()
