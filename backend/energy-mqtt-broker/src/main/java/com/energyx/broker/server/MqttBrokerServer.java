 ...
 * </ol>
 * 持久会话状态（订阅/inflight/离线队列）已在 Redis，设备重连其他节点即可接管。
 * </p>
 */
@Slf4j
@Component
public class MqttBrokerServer {
	...
	/**
	 * 两个同类型 {@link ServerBootstrap} Bean（明文 + TLS）必须按名限定注入； TLS bootstrap 仅在
	 * {@code energyx.broker.tls.enabled=true} 时存在，用 {@link ObjectProvider} 惰性获取，TLS 关闭时
	 * {@code getIfAvailable()} 返回 null。
	 */
	public MqttBrokerServer(@Qualifier("mqttServerBootstrap") ServerBootstrap bootstrap,
			...
			BrokerProperties properties, SessionRegistry sessionRegistry, RouterConsumer routerConsumer,
			KafkaTopicInitializer topicInitializer, KafkaEventProducer kafkaProducer,
			RetainedMessageStore retainedStore, ExecutorService brokerExecutor) {
