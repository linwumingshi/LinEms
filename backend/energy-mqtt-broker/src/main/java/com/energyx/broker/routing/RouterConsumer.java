 ...
 *
 * <p>三通道（每通道独立消费组、独立线程池，互不阻塞）：
 * <ul>
 *   <li><b>mqtt.down.{nodeId}</b>：节点专属下行（access/跨节点 Broker 定向写入），
 *       消费组 mqtt-down-{nodeId}，仅本节点能收到；PUBLISH → deliverToSession（本节点即目标）；</li>
 *   <li><b>mqtt.broadcast</b>：跨节点广播（KICK 踢线、owner 解析失败/离线回落），
 *       消费组 mqtt-bc-{nodeId} 每节点唯一 → 全量 fan-out + sourceNode 去重；</li>
 *   <li><b>mqtt.router（兼容期）</b>：旧 fan-out 通道，仅 router-legacy-broadcast=true 时启用。</li>
 * </ul>
 *
 * <p>消费语义：手动提交（整批处理完 commitSync）+ earliest + 多线程分区并行（单分区保序），
 * 单条失败不阻塞分区（记日志 + 进 DLQ 后照常提交，at-least-once）。</p>
 *
 * <p>信封编码：二进制（RouterEnvelopeCodec，magic 0xE9 0x01）；兼容期通道同时接受 JSON（自动探测）。</p>
 */
@Slf4j
@Component
public class RouterConsumer implements BytesKafkaRecordHandler {
    ...
    public synchronized void start() {
        ...
        downEngine = new BytesKafkaConsumerEngine(
                List.of(downTopic), "mqtt-down-" + properties.getNodeId(),
                this, baseProps(), threads, 100, this::dlq, dlq);
        downEngine.start();
        // 2. 跨节点广播：每节点唯一消费组全量 fan-out
        broadcastEngine = new BytesKafkaConsumerEngine(
                List.of(KafkaTopicConstant.MQTT_BROADCAST), "mqtt-bc-" + properties.getNodeId(),
                this, baseProps(), threads, 100, this::dlq, dlq);
        broadcastEngine.start();
        // 3. 兼容期旧通道（默认关闭）
        if (properties.isRouterLegacyBroadcast()) {
            legacyEngine = new BytesKafkaConsumerEngine(
                    List.of(KafkaTopicConstant.MQTT_ROUTER), "mqtt-router-" + properties.getNodeId(),
                    this, baseProps(), threads, 100, this::dlq, dlq);
            legacyEngine.start();
        }
        started = true;
        log.info("[Router] 路由消费者启动 down={} broadcast={} legacy={} threads={}",
                downTopic, KafkaTopicConstant.MQTT_BROADCAST,
    ...
    public void handle(ConsumerRecord<String, byte[]> record) {
        ...
        try {
            ...
            log.warn("[Router] 信封反序列化失败 topic={} key={} offset={}，丢弃",
