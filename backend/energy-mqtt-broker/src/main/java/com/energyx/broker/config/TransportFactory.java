 ...
 *
 * <p>优先级：Epoll（Linux，SO_REUSEPORT/零拷贝/批量唤醒）→ KQueue（macOS，反射，本地仓库
 * 无构件故不引入编译期依赖）→ NIO（回退）。探测基于 {@code netty-transport-native-*}
 * 是否在 classpath 且当前平台可用（{@code Epoll.isAvailable()}）；optional 依赖缺失时
 * 自动回退 NIO，Windows 开发机恒为 NIO（功能验证足够）。</p>
 *
 * <p>生产 Linux 部署：pom 内 optional 的 epoll 依赖改为非 optional（或运行时注入
 * netty-transport-native-epoll:linux-x86_64 jar），本工厂自动启用 epoll；无原生 jar 时
 * 打印告警并回退 NIO，不阻断启动。</p>
 */
@Slf4j
        ...
        synchronized (TransportFactory.class) {
            ...
            try {
                if (Epoll.isAvailable()) {
                    ...
                    log.warn("[Transport] 无原生传输可用（epoll 未就绪：{}），回退 NIO。"
