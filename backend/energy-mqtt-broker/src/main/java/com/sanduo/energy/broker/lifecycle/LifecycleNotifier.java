package com.sanduo.energy.broker.lifecycle;
 ...
 * 设备生命周期通知（上线/下线）。
 *
 * <p>双通道（Phase 1 §4.4 / Redis-key规范 §3.1）：
 * <ul>
...
public class LifecycleNotifier {
    ...
    public LifecycleNotifier(SessionStore sessionStore, KafkaEventProducer producer,
                             BrokerProperties properties, ObjectMapper objectMapper) {
    ...
    private void publish(DeviceCredential cred, String eventType, String reason, String ip) {
        ...
        try {
            producer.send(KafkaTopicConstant.IOT_DEVICE_LIFECYCLE,
                    String.valueOf(cred.getDeviceId()), objectMapper.writeValueAsString(event));
        } catch (Exception e) {
