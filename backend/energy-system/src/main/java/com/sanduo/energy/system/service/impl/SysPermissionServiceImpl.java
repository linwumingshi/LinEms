 ...
 * 菜单资源服务：目录/菜单/按钮树维护。
 *
 * <p>权限标识（perm_code）唯一；写操作后调用 {@link TokenService#refreshAllSessions}
 * 刷新全部在线会话，使菜单/按钮增删改、停用即时生效。</p>
 */
@Slf4j
@Service
public class SysPermissionServiceImpl extends ServiceImpl<SysPermissionMapper, SysPermission>
    ...
    public void deletePermission(Long permId) {
        ...
        long children = count(new LambdaQueryWrapper<SysPermission>()
                .eq(SysPermission::getParentId, permId));
        if (children > 0) {
