...
import com.sanduo.energy.system.dto.SysEnterpriseSaveReq;
...
import com.sanduo.energy.system.service.SysEnterpriseService;
 ...
 *
 * <p>path 约定：根节点 "/{id}/"，子节点 = 父 path + "{id}/"，如 "/1/3/"；
 * 父级移动时按路径前缀级联修正整棵子树，保证 likeRight 查询始终可用。</p>
 */
@Slf4j
@Service
public class SysEnterpriseServiceImpl extends ServiceImpl<SysEnterpriseMapper, SysEnterprise>
    ...
    public List<SysEnterprise> tree() {
        ...
        Map<Long, SysEnterprise> byId = all.stream()
    ...
    public Long createEnterprise(SysEnterpriseSaveReq req) {
        ...
        long count = count(new LambdaQueryWrapper<SysEnterprise>()
                .eq(SysEnterprise::getTenantId, tenantId)
        ...
        updateById(entity);
        log.info("创建单位 enterpriseId={} code={} path={}", entity.getEnterpriseId(),
                entity.getEnterpriseCode(), entity.getPath());
    ...
    public void updateEnterprise(Long enterpriseId, SysEnterpriseSaveReq req) {
        ...
        if (!Objects.equals(exists.getEnterpriseCode(), req.getEnterpriseCode())) {
            long count = count(new LambdaQueryWrapper<SysEnterprise>()
                    .eq(SysEnterprise::getTenantId, exists.getTenantId())
                    .eq(SysEnterprise::getEnterpriseCode, req.getEnterpriseCode())
    ...
    public void deleteEnterprise(Long enterpriseId) {
        ...
        long children = count(new LambdaQueryWrapper<SysEnterprise>()
                .eq(SysEnterprise::getParentId, enterpriseId));
        if (children > 0) {
    ...
    private List<SysEnterprise> orderedList() {
        return list(new LambdaQueryWrapper<SysEnterprise>()
                .orderByAsc(SysEnterprise::getSort)
    ...
    private void cascadeChildren(SysEnterprise self, String oldPath, int oldLevel) {
        List<SysEnterprise> children = list(new LambdaQueryWrapper<SysEnterprise>()
                .likeRight(SysEnterprise::getPath, oldPath)
