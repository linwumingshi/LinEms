 ...
 *
 * <p>
 * 多租户限定登录名：sys_user 唯一键为 (tenant_id, username)，故登录主体采用 复合主键格式 {@code tenantId:username}，由
 * {@code AuthServiceImpl} 构造、本类解析。 账号不存在抛 {@link UsernameNotFoundException}（Provider 统一转
 * BadCredentialsException， 防账号枚举）；禁用/锁定不在此抛异常，交由 Provider 校验
...
public class UserDetailsServiceImpl implements UserDetailsService {
	...
	public LoginUser loadUserByUsername(String principal) throws UsernameNotFoundException {
		...
		SysUser user = userMapper
			.selectOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getTenantId, Long.parseLong(tenantAndName[0]))
		...
		return new LoginUser(user.getUserId(), user.getTenantId(), user.getEnterpriseId(), user.getRealName(),
				user.getUsername(), auth.permissions(), auth.roleCodes(), user.getPassword(), user.getStatus());
	}
