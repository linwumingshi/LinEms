 ...
 * 用户服务：CRUD + 角色分配。
 *
 * <p>安全约束：超级管理员（userId=1）与当前登录账号不可删除/禁用；
 * 密码变更/禁用/删除吊销在线会话（TokenService.revokeUserSessions）；
 * 角色分配后刷新在线会话身份（TokenService.refreshUserSessions）。</p>
 */
@Slf4j
@Service
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
    ...
    public SysUserServiceImpl(SysUserRoleMapper userRoleMapper, SysRoleMapper roleMapper,
                              SysEnterpriseMapper enterpriseMapper, PasswordEncoder passwordEncoder,
                              TokenService tokenService, PermissionResolver permissionResolver) {
    ...
    public void updateUser(Long userId, SysUserSaveReq req) {
        ...
        if (!Objects.equals(exists.getUsername(), req.getUsername())) {
            long count = count(new LambdaQueryWrapper<SysUser>()
                    .eq(SysUser::getTenantId, exists.getTenantId())
                    .eq(SysUser::getUsername, req.getUsername())
    ...
    private void fillEnterpriseNames(List<SysUserVO> vos) {
        Set<Long> ids = vos.stream().map(SysUserVO::getEnterpriseId)
                .filter(Objects::nonNull).collect(Collectors.toSet());
        if (ids.isEmpty()) {
            return;
        }
        Map<Long, SysEnterprise> map = enterpriseMapper.selectBatchIds(ids).stream()
    ...
    private void fillRoles(List<SysUserVO> vos) {
        ...
        Map<Long, List<Long>> userRoleMap = userRoleMapper.selectList(
