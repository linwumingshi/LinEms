 ...
 *
 * <p>
 * JWT 仅作为签名会话标识（携带 sid 指向 Redis），完整身份与权限缓存在 {@code auth:login_token:{sid}}；登出删除 Redis
 * 键即吊销会话，角色权限变更可实时刷新。
 * </p>
 *
 * <p>
 * 注：会话全量扫描使用 Redis KEYS（对齐若依），在线会话量大时建议升级为 SCAN 游标 分片遍历（见 Phase9 容量项）。
 * </p>
 */
@Slf4j
@Component
public class TokenService {
	...
	public String createToken(LoginUser loginUser) {
		...
		JwtClaims claims = new JwtClaims(loginUser.getUserId(), loginUser.getUsername(), loginUser.getTenantId(),
				loginUser.getEnterpriseId(), loginUser.getRealName(), sid);
	...
	public void refreshPermissionByRoleCode(String roleCode, PermissionResolver resolver) {
		...
		for (String key : keys) {
			...
			if (loginUser == null || loginUser.getRoleCodes() == null || !loginUser.getRoleCodes().contains(roleCode)) {
				continue;
			}
	...
	/**
	 * 权限资源变更后全量刷新在线会话权限（角色集合不变，仅重算权限标识）。 用于菜单/按钮增删改或停用后立即生效。
	...
	private void refreshToken(LoginUser loginUser) {
		loginUser.setLoginTime(System.currentTimeMillis());
		loginUser.setExpireTime(loginUser.getLoginTime() + jwtProperties.getExpireSeconds() * 1000L);
		redisUtils.setJson(key(loginUser.getToken()), loginUser, Duration.ofSeconds(jwtProperties.getExpireSeconds()));
	}
