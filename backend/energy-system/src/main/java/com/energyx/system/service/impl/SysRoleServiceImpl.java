 ...
 *
 * <p>
 * 权限分配后调用 {@link TokenService#refreshPermissionByRoleCode} 刷新持有该角色的在线会话，
 * 使权限变更立即生效；内置超级管理员角色（roleId=1）不可删除/禁用。
 * </p>
 */
@Slf4j
@Service
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements SysRoleService {
	...
	public SysRoleServiceImpl(SysRolePermissionMapper rolePermissionMapper, SysUserRoleMapper userRoleMapper,
			SysPermissionMapper permissionMapper, TokenService tokenService, PermissionResolver permissionResolver) {
	...
	public Long createRole(SysRoleSaveReq req) {
		...
		long count = count(new LambdaQueryWrapper<SysRole>().eq(SysRole::getTenantId, tenantId)
	...
	public void updateRole(Long roleId, SysRoleSaveReq req) {
		...
		if (!Objects.equals(exists.getRoleCode(), req.getRoleCode())) {
			long count = count(new LambdaQueryWrapper<SysRole>().eq(SysRole::getTenantId, exists.getTenantId())
				.eq(SysRole::getRoleCode, req.getRoleCode())
	...
	public List<Long> permIds(Long roleId) {
		return rolePermissionMapper
			.selectList(new LambdaQueryWrapper<SysRolePermission>().eq(SysRolePermission::getRoleId, roleId))
			.stream()
