...
public class TdenginePlanWriter {
	...
	/**
	 * 写时间戳用 JVM 默认时区换算 epoch 毫秒：TDengine 把裸 datetime 字符串按 UTC 解析（会 +8 偏移）， epoch 毫秒 + 读侧
	 * getTime(ts).toLocalTime()（同样按 JVM 时区）可无偏移往返
	 */
	...
	public void write(long stationId, LocalDate planDate, List<PlanPoint> points) throws Exception {
		...
		try (Connection conn = DriverManager.getConnection(jdbcUrl, username, password);
			...
			st.execute("CREATE STABLE IF NOT EXISTS ems_plan_point "
					+ "(ts TIMESTAMP, action VARCHAR(16), power_kw DOUBLE, soc DOUBLE) " + "TAGS (station_id BIGINT)");
			...
			st.execute("DELETE FROM ems_plan_point " + "WHERE station_id = " + stationId + " AND ts >= " + start
					+ " AND ts < " + end);
			...
			StringBuilder sb = new StringBuilder("INSERT INTO ").append(table)
				.append(" USING ems_plan_point TAGS (")
				.append(stationId)
				.append(") ")
	...
	public List<PlanPoint> read(long stationId, LocalDate planDate) throws Exception {
		...
		try (Connection conn = DriverManager.getConnection(jdbcUrl, username, password);
				Statement st = conn.createStatement();
				ResultSet rs = st.executeQuery("SELECT ts, action, power_kw, soc FROM plan_" + stationId
						+ " WHERE ts >= " + start + "   AND ts <  " + end + " ORDER BY ts")) {
			while (rs.next()) {
				java.sql.Time tm = rs.getTime("ts");
				out.add(new PlanPoint(tm.toLocalTime(), rs.getString("action"), rs.getDouble("power_kw"),
						rs.getDouble("soc")));
			}
		}
		return out;
