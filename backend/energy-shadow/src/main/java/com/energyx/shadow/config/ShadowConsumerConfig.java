...
import com.energyx.shadow.consumer.PropertyShadowConsumer;
 ...
 *
 * <p>
 * 消费 iot-thing-property（key=deviceId，单分区单线程 ⇒ 单设备保序）， 失败记录进 iot-dlq；因影子合并且有乐观锁，不设消息去重（详见
 * PropertyShadowConsumer 注释）。
 * </p>
 */
@Configuration
public class ShadowConsumerConfig {
	...
	public KafkaConsumerEngine propertyShadowEngine(PropertyShadowConsumer handler, ShadowProperties props,
		...
		KafkaConsumerEngine engine = new KafkaConsumerEngine(KafkaTopicConstant.IOT_THING_PROPERTY,
				"energy-shadow-prop", handler, baseProps(props), props.getConsumerThreads(), props.getPollMs(),
