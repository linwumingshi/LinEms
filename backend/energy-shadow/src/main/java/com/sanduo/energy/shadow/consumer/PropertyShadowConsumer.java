package com.sanduo.energy.shadow.consumer;
 ...
 *
 * <p><b>刻意不做消息级去重</b>：影子合并且有版本乐观锁，天然幂等。若先写 dedup key 再合并，
 * 一旦合并过程中 Redis 成功 / MySQL 失败，Kafka 重放会被去重直接跳过，MySQL 永不收敛；
 * 不设去重则重放重跑合并，自我收敛。缺陷仅是同一设备同值快照的重复合并（结果不变），可接受。</p>
 */
@Slf4j
@Component
