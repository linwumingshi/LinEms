package com.sanduo.energy.shadow.delta;
...
/**
 * delta 发布：差异非空时投递 iot-shadow-delta（key=deviceId，消费方：指令中心物化为 set 指令 /
 * ws-pusher 驾驶舱刷新），并写 Redis 短 TTL 的 delta 快照供设备上线拉取。
 */
@Slf4j
@Component
public class ShadowDeltaPublisher {
    ...
    public ShadowDeltaPublisher(ShadowKafkaProducer producer, StringRedisTemplate redis,
