package com.sanduo.energy.shadow.service;

import com.fasterxml.jackson.core.type.TypeReference;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.sanduo.energy.shadow.config.ShadowProperties;
import com.sanduo.energy.shadow.delta.DeltaCalculator;
import com.sanduo.energy.shadow.delta.ShadowDeltaPublisher;
import com.sanduo.energy.shadow.mapper.ShadowHistoryMapper;
import com.sanduo.energy.shadow.mapper.ShadowMapper;
import com.sanduo.energy.shadow.model.ShadowRow;
import com.sanduo.energy.shadow.util.ShadowRedisKeys;
 ...
 *
 * <p><b>幂等性设计</b>：影子合并是<b>天然幂等</b>的操作——同一属性快照重复合并结果不变，
 * 因此消费端不做消息级去重（避免"部分失败后重放被去重跳过导致 MySQL 永不收敛"），
 * 依赖 Kafka at-least-once 重放 + 乐观锁重试自我收敛。desired 由版本乐观锁防并发覆盖。</p>
 */
@Slf4j
@Service
public class ShadowService {
    ...
    public ShadowService(ShadowMapper shadowMapper, ShadowHistoryMapper historyMapper,
                         StringRedisTemplate redis, ShadowDeltaPublisher deltaPublisher,
                         ObjectMapper objectMapper, ShadowProperties props) {
    ...
    /**
     * 设备属性上报 → 影子 reported 双写。
     * 合并语义：新上报的属性覆盖旧值，未上报的旧属性保留（部分上报不丢失上下文）。
     */
    ...
    private void maybeWriteHistory(long deviceId, String snapshot, int version, int operatorType) {
        ...
        Boolean acquired = redis.opsForValue().setIfAbsent(
                ShadowRedisKeys.historyThrottle(deviceId), "1",
    ...
    private Map<String, Object> readHashAsMap(String key) {
        // Spring Data Redis 3.x 的 opsForHash() 泛型签名为 <K,Object,Object>，entries 返回 Map<Object,Object>
