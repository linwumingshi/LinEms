...
public class CommandConsumerConfig {
	...
	public KafkaConsumerEngine ackEngine(AckCommandConsumer handler, CommandProperties props,
		...
		return start(new KafkaConsumerEngine(KafkaTopicConstant.IOT_COMMAND_ACK, "energy-cmd-ack", handler,
	...
	public KafkaConsumerEngine deltaEngine(DeltaCommandConsumer handler, CommandProperties props,
			CommandKafkaProducer producer) {
		return start(new KafkaConsumerEngine(KafkaTopicConstant.IOT_SHADOW_DELTA, "energy-cmd-delta", handler,
				baseProps(props), props.getConsumerThreads(), props.getPollMs(), producer::send, props.getDlqTopic()));
	}
	...
	public KafkaConsumerEngine lifecycleEngine(LifecycleCommandConsumer handler, CommandProperties props,
			CommandKafkaProducer producer) {
		return start(new KafkaConsumerEngine(KafkaTopicConstant.IOT_DEVICE_LIFECYCLE, "energy-cmd-lifecycle", handler,
				baseProps(props), props.getConsumerThreads(), props.getPollMs(), producer::send, props.getDlqTopic()));
	}
