package com.energyx.command.mqtt;
...
/**
 * 指令侧 Kafka 生产者：发布 iot-command-down（下行指令，key=deviceId）。 idempotent
 * producer（enable.idempotence + acks=all），与 access/tsdb/shadow 参数一致。
 */
@Slf4j
@Component
