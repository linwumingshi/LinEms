package com.sanduo.energy.command.service;

import com.fasterxml.jackson.core.type.TypeReference;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.sanduo.energy.command.config.CommandProperties;
import com.sanduo.energy.command.mapper.CommandAckMapper;
import com.sanduo.energy.command.mapper.CommandMapper;
import com.sanduo.energy.command.mapper.DeviceInfoMapper;
import com.sanduo.energy.command.model.CommandRow;
import com.sanduo.energy.command.model.DeviceInfo;
import com.sanduo.energy.command.mqtt.CommandKafkaProducer;
import com.sanduo.energy.command.state.CommandState;
import com.sanduo.energy.command.util.CommandRedisKeys;
import com.sanduo.energy.command.web.dto.CommandView;
import com.sanduo.energy.command.web.dto.CreateCommandRequest;
import com.sanduo.energy.common.constant.KafkaTopicConstant;
import com.sanduo.energy.common.message.CommandAckMessage;
import com.sanduo.energy.common.message.CommandDownMessage;
import com.sanduo.energy.common.message.ShadowDeltaMessage;
import com.sanduo.energy.common.redis.IdempotencyUtils;
 ...
 * </p>
 *
 * <p><b>在线/离线分流</b>：在线直发 Kafka（iot-command-down）+ 置 SENT；离线入队 iot:cmd:q（保持 CREATED），
 * 设备上线（lifecycle ONLINE）触发补发。ACK 超时由 @Scheduled 扫描驱动重试/终态。</p>
 */
@Slf4j
@Service
public class CommandService {
    ...
    public CommandService(CommandMapper commandMapper, CommandAckMapper ackMapper,
                          DeviceInfoMapper deviceMapper, StringRedisTemplate redis,
                          CommandKafkaProducer producer, ObjectMapper objectMapper,
                          IdempotencyUtils idempotencyUtils, CommandProperties props,
    ...
    /**
     * 创建指令：解析设备 → 落库（CREATED）→ 在线直发 / 离线入队。
     * commandId 为空时服务端生成；传入时作为幂等键（重复创建返回既有指令）。
     */
    public CommandView createCommand(CreateCommandRequest req) {
        String commandId = req.getCommandId() == null || req.getCommandId().isBlank()
                ? idGenerator.nextIdStr() : req.getCommandId();
        ...
        try {
            ...
            commandMapper.insert(commandId, dev.tenantId(), dev.deviceId(), dev.productKey(),
                    req.getCommand(), commandType, toJson(req.getParams()),
                    maxRetry, timeoutMs, req.getCreateBy() == null ? 0L : req.getCreateBy());
    ...
    /**
     * ACK 状态机收敛（消费 iot-command-ack）。
     * 条件更新天然幂等：终态忽略 / 非法转移忽略 / 重放空操作。
     */
    ...
    /**
     * 影子 delta → setProperties 指令（消费 iot-shadow-delta）。
     * 同设备存在在途 setProperties 则合并跳过（shadow 收敛后再发新 delta）。
     */
    public void materializeDelta(ShadowDeltaMessage delta) {
        ...
        if (inflight != null) {
            log.info("[Command] 存在在途 setProperties 指令 commandId={}，合并 delta deviceId={}",
                    inflight, dev.deviceId());
            return;
        }
        String commandId = idGenerator.nextIdStr();
        commandMapper.insert(commandId, dev.tenantId(), dev.deviceId(), dev.productKey(),
                DELTA_COMMAND_NAME, 2, toJson(delta.getDesired()),
                props.getDefaultMaxRetry(), props.getDefaultTimeoutMs(), 0L);
        dispatch(commandId, dev);
        log.info("[Command] delta 物化为 setProperties 指令 commandId={} deviceId={} keys={}",
                commandId, dev.deviceId(), delta.getDesired().size());
    }
    ...
    public void drainOfflineQueue(long deviceId) {
        ...
        int drained = 0;
        String json;
        while (drained < props.getOfflineQueueDrainMax()
                && (json = redis.opsForList().leftPop(key)) != null) {
            drained++;
            try {
    ...
    private void retryOnce(CommandRow row) {
        ...
        if (isOnline(row.getDeviceId())) {
            ...
            if (dev != null) {
                producer.send(KafkaTopicConstant.IOT_COMMAND_DOWN,
                        String.valueOf(row.getDeviceId()), toJson(buildDown(row, dev)));
                markInflight(row.getDeviceId(), row.getCommandId());
            }
            log.info("[Command] 超时重发 commandId={} retry={}/{}", row.getCommandId(),
                    row.getRetryCount() + 1, row.getMaxRetry());
        } else {
            ...
            log.info("[Command] 超时重新入队 commandId={} retry={}/{}", row.getCommandId(),
                    row.getRetryCount() + 1, row.getMaxRetry());
        }
    }
    ...
    private void dispatch(String commandId, DeviceInfo dev) {
        ...
        if (isOnline(dev.deviceId())) {
            ...
            log.info("[Command] 在线直发 commandId={} deviceId={} command={}",
                    commandId, dev.deviceId(), row.getCommandName());
        } else {
            enqueueOffline(row, dev);
        }
    }
    ...
    private void markInflight(long deviceId, String commandId) {
        try {
            redis.opsForHash().put(CommandRedisKeys.inflight(deviceId), commandId,
