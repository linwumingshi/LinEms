 ...
 *
 * <p>由告警服务（energy-alarm）在规则命中/恢复时产出，消费方：ws-pusher（驾驶舱实时弹窗）、
 * 通知中心（短信/邮件）、运营大屏、ES 日志落库。status 取值：ACTIVE（触发）/ RECOVERED（恢复）。</p>
 *
 * <p>ACK（人工确认）不进消息总线——记录状态变更由 REST 直改 iot_alarm_record，告警查询以 DB 为准，
 * 避免与规则引擎的状态机耦合。</p>
 */
@Data
