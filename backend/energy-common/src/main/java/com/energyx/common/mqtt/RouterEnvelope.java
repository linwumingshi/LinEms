 ...
 *
 * <p>
 * 该信封是「Broker 集群 ↔ 接入适配」之间的跨模块契约，因此沉淀在 common：
 * <ul>
 * <li>Broker 侧：设备上行 PUBLISH 跨节点 fan-out、同 clientId 远端踢线（KICK）；</li>
 * <li>接入适配侧：消费 mqtt.router 解析设备上行；平台下行指令也以 PUBLISH 信封 写回 mqtt.router，经 Broker 全节点 fan-out
 * 投递给订阅 down/* 的设备。</li>
 * </ul>
 * </p>
 *
 * <p>
 * 消费组约定（Phase 1 §4.7）：Broker 每节点唯一消费组 `mqtt-router-{nodeId}` 实现全量 fan-out 并按 sourceNode
 * 去重；接入适配以独立消费组（energy-access-uplink）获取「恰好一次」副本。
 * </p>
 */
@Data
public class RouterEnvelope {
	...
	public static RouterEnvelope publish(String sourceNode, String topic, byte[] payload, int qos, boolean retain) {
	...
	public void setPayload(byte[] payload) {
		this.payloadBase64 = payload == null || payload.length == 0 ? "" : Base64.getEncoder().encodeToString(payload);
	}
