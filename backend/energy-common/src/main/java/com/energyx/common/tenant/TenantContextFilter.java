...
/**
 * 租户上下文过滤器：从网关透传头 {@code x-tenant-id} / {@code x-enterprise-id} 读取当前租户， 注入
 * {@link TenantContext}，请求结束后必须 clear（防 ThreadLocal 泄漏到线程池复用线程）。
 *
 * <p>
 * Order 在 {@link com.energyx.common.web.TraceFilter}（HIGHEST_PRECEDENCE）之后； 无头（内部调用 /
 * 网关白名单路径）不设置上下文，租户拦截器据此跳过全部表。
 * </p>
 */
@Component
