 ...
 *
 * <p>
 * 二进制格式（magic=0xE9 0x01，与 JSON（首字节 '{'=0x7B）天然可区分）： <pre>
 * 偏移  长度  字段
 * 0     2    magic 0xE9 0x01
 * 2     1    version = 1
 ...
 *
 * <p>
 * 兼容策略：decode 自动探测 magic——二进制走快速路径；JSON 走 ObjectMapper （兼容期 mqtt.router
 * 旧通道与测试），平滑滚动升级无需双端同时切换。
 * </p>
 */
public final class RouterEnvelopeCodec {
	...
	public static boolean isBinary(byte[] data) {
		return data != null && data.length >= 2 && data[0] == MAGIC[0] && data[1] == MAGIC[1];
	}
