...
/** 告警规则状态（iot_alarm_rule.status，DB 存 code） */
public enum AlarmRuleStatus {
	...
	AlarmRuleStatus(int code, String desc) {
		this.code = code;
		this.desc = desc;
	}
	...
	@JsonCreator
	public static AlarmRuleStatus fromCode(int code) {
		return Arrays.stream(values())
			.filter(e -> e.code == code)
			.findFirst()
			.orElseThrow(() -> new IllegalArgumentException("未知 AlarmRuleStatus code=" + code));
	}

	/** 安全查找：未知/空返回 null（查询/宽容场景用，避免抛错） */
	public static AlarmRuleStatus of(Integer code) {
		if (code == null) {
			return null;
		}
