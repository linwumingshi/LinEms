 ...
 *
 * <p>
 * 为什么按 stage 区分命名空间：一条设备报文会顺序经过多个消费边界（access 摄入 → tsdb 落库 → shadow/rule 处理），每个边界的 Kafka
 * 交付都可能各自重放。若共用同一 key，上游边界一旦消费过， 下游边界会把合法消息误判为重复而丢弃。因此每个边界独立 SETNX、独立 TTL。
 * </p>
 *
 * <p>
 * 用法：{@code messageDedup.tryOnce("access", deviceId, messageId, 300)} 返回
 * true=首次，可继续；false=重复。
 * </p>
 */
@Slf4j
@Component
