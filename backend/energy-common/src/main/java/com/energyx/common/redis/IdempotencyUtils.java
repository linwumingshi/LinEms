...
/**
 * 幂等工具：SETNX 实现"一次性"语义。 用于指令去重（commandId）、消息去重（messageId）、影子 delta 幂等（Phase 6）。
 */
@Slf4j
@Component
public class IdempotencyUtils {
	...
	public boolean tryAcquire(String key, long ttlSeconds) {
		return Boolean.TRUE.equals(redis.opsForValue().setIfAbsent(PREFIX + key, "1", Duration.ofSeconds(ttlSeconds)));
	}
