...
/** 电站运行状态（iot_station.status，DB 存 code） */
public enum StationStatus {

	STOPPED(0, "停运"), RUNNING(1, "运行");
	...
	StationStatus(int code, String desc) {
		this.code = code;
		this.desc = desc;
	}
	...
	@JsonCreator
	public static StationStatus fromCode(int code) {
		return Arrays.stream(values())
			.filter(e -> e.code == code)
			.findFirst()
			.orElseThrow(() -> new IllegalArgumentException("未知 StationStatus code=" + code));
	}

	/** 安全查找：未知/空返回 null（查询/宽容场景用，避免抛错） */
	public static StationStatus of(Integer code) {
		if (code == null) {
			return null;
		}
