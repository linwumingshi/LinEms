...
/** 安全包络约束状态（ems_constraint.status，DB 存 code；schema 默认 1 启用） */
public enum ConstraintStatus {
	...
	ConstraintStatus(int code, String desc) {
		this.code = code;
		this.desc = desc;
	}
	...
	@JsonCreator
	public static ConstraintStatus fromCode(int code) {
		return Arrays.stream(values())
			.filter(e -> e.code == code)
			.findFirst()
			.orElseThrow(() -> new IllegalArgumentException("未知 ConstraintStatus code=" + code));
	}

	/** 安全查找：未知/空返回 null（查询/宽容场景用，避免抛错） */
	public static ConstraintStatus of(Integer code) {
		if (code == null) {
			return null;
		}
