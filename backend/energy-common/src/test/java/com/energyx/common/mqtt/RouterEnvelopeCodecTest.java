...
class RouterEnvelopeCodecTest {
	...
	void publishRoundTrip() {
		...
		RouterEnvelope env = RouterEnvelope.publish("broker-1", "pk1/dev1/up/property", payload, 1, true);
	...
	void truncatedBinaryFailsFast() {
		...
		assertThrows(Exception.class, () -> RouterEnvelopeCodec.decode(truncated, objectMapper), "截断报文必须快速失败而非静默越界");
	}
