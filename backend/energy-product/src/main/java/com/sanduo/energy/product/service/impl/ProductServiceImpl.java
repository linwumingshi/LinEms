package com.sanduo.energy.product.service.impl;
...
import com.sanduo.energy.product.entity.ThingModel;
import com.sanduo.energy.product.mapper.ProductMapper;
import com.sanduo.energy.product.mapper.ThingModelMapper;
import com.sanduo.energy.product.service.ProductService;
import com.sanduo.energy.product.util.ThingModelValidator;
 ...
 * 本服务仅在校验时读取当前租户写入。物模型发布：同版本覆盖、异版本新增并切换 is_current。</p>
 */
@Slf4j
@Service
public class ProductServiceImpl extends ServiceImpl<ProductMapper, Product> implements ProductService {
    ...
    public Long create(ProductSaveReq req) {
        ...
        long dup = count(new LambdaQueryWrapper<Product>()
                .eq(Product::getProductKey, req.getProductKey()));
        if (dup > 0) {
        ...
        save(product);
        log.info("创建产品 productId={} key={} name={}", product.getProductId(),
    ...
    public ThingModelView getThingModel(Long productId) {
        ...
        ThingModel model = thingModelMapper.selectOne(new LambdaQueryWrapper<ThingModel>()
                .eq(ThingModel::getProductId, productId)
    ...
    public ThingModelView saveThingModel(Long productId, ThingModelSaveReq req) {
        ...
        ThingModel exist = thingModelMapper.selectOne(new LambdaQueryWrapper<ThingModel>()
                .eq(ThingModel::getProductId, productId)
        ...
        thingModelMapper.update(null, new LambdaUpdateWrapper<ThingModel>()
                .eq(ThingModel::getProductId, productId)
